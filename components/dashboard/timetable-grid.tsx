    ...
    <Card className="border border-border bg-white shadow-sm rounded-lg overflow-hidden">
      <CardHeader className="bg-slate-50 border-b border-border flex flex-col md:flex-row md:items-center justify-between gap-6 py-8 px-10">
        <div className="space-y-1">
          ...
          <p className="micro-text text-muted-foreground font-semibold uppercase tracking-widest opacity-60">Manage class slots and teacher assignments</p>
        </div>
        <div className="flex items-center gap-4 bg-white px-6 py-3 rounded-md border border-border shadow-inner">
          ...
          <Label htmlFor="eid-mode" className="text-[13px] font-semibold text-foreground cursor-pointer uppercase tracking-tight">
            Eid Schedule
          </Label>
          <Switch 
            id="eid-mode" 
            ...
            <div key={slot.id} className="space-y-6 animate-in fade-in slide-in-from-top-1 duration-300" style={{ animationDelay: `${index * 50}ms` }}>
              <div className="flex items-center justify-between border-b border-border pb-3">
                <div className="flex items-center gap-3">
                  <Badge variant="outline" className="text-primary border-primary/20 font-semibold px-2 py-0.5 text-[11px]">SLOT {slot.id}</Badge>
                ...
                {isEidMode && <Badge className="bg-warning text-white text-[9px] font-semibold uppercase tracking-tighter">SHIFTED</Badge>}
              </div>
              ...
              <div className="space-y-3">
                {scheduleData[index].map((item, i) => (
                  <div key={i} className="group flex items-center gap-4 p-4 rounded-md border border-border bg-white hover:bg-slate-50 transition-all duration-200">
                    <div className="flex flex-col items-center justify-center w-12 h-12 rounded-md bg-slate-100 border border-slate-200 font-semibold text-slate-800 transition-colors group-hover:bg-primary/5 group-hover:border-primary/20 group-hover:text-primary">
                      <span className="text-[10px] uppercase font-semibold tracking-tighter opacity-40 leading-none mb-1">Room</span>
                      <span className="text-[16px] leading-none">{item.room}</span>
                    </div>
                    <div className="flex-1 min-w-0 space-y-1">
                      <div className="flex items-center gap-2">
                        <p className="text-[14px] font-semibold text-foreground truncate">{item.level}</p>
                      </div>
                      <div className="flex items-center gap-2">
