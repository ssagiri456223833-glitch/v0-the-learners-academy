...
import { Trophy, Target, CheckCircle, BookOpen, PenTool, Type, HelpCircle, Lightbulb, AlertTriangle, Flame, Activity, ShieldCheck, ArrowRight } from "lucide-react"
            ...
            <div className="flex-1 text-center md:text-left space-y-8">
              <div className="space-y-4">
                 <div className="flex items-center justify-center md:justify-start gap-3">
                    ...
                    <span className="micro-text text-muted-foreground font-semibold uppercase tracking-[0.25em] opacity-40">Test Complete • Answers Saved</span>
                 </div>
                 <h2 className="page-title text-[42px] text-foreground leading-tight tracking-tight italic underline decoration-primary/20 underline-offset-8 decoration-2">
                  Assessment <span className="opacity-40 not-italic">Results</span>
                </h2>
                <div className="text-[24px] font-semibold text-primary italic opacity-80 mt-2">
                  {getStatusLabel()}
                </div>
              </div>
              ...
              <div className="flex flex-wrap justify-center md:justify-start gap-12 pt-8 border-t border-border/60">
                <div className="flex flex-col items-start gap-2 group transition-all duration-300">
                  <div className="flex items-center gap-2">
                     ...
                     <span className="micro-text text-muted-foreground font-semibold uppercase tracking-widest opacity-40">Correct Answers</span>
                  </div>
                  <span className="text-[20px] font-semibold italic tabular-nums text-foreground">{score} <span className="text-[14px] not-italic opacity-20">SCORE</span></span>
                </div>
                <div className="flex flex-col items-start gap-2 group transition-all duration-300">
                  <div className="flex items-center gap-2">
                     <Target className="h-4 w-4 text-primary opacity-60" />
                     <span className="micro-text text-muted-foreground font-semibold uppercase tracking-widest opacity-40">Total Questions</span>
                  </div>
        ...
        <Card className="border border-border bg-white shadow-sm rounded-lg lg:col-span-2 overflow-hidden">
          <div className="bg-slate-50 border-b border-border py-6 px-10 flex items-center justify-between">
             <div className="flex items-center gap-3">
                ...
                <h3 className="page-title text-[20px] text-foreground italic tracking-tight underline decoration-primary/10 underline-offset-4 decoration-1">Skill Breakdown</h3>
             </div>
             <span className="micro-text text-muted-foreground font-semibold uppercase tracking-widest opacity-30">Performance Details</span>
          </div>
                ...
                <div key={skill.name} className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[14px] font-semibold uppercase tracking-widest text-foreground opacity-80">{skill.name}</span>
                    </div>
                    <span className="text-[16px] font-semibold italic tabular-nums text-primary">{skill.score}<span className="text-[11px] not-italic opacity-20 mx-1">/</span>{skill.total}</span>
                  </div>
                  <Progress value={(skill.score / skill.total) * 100} className="h-1.5 rounded-full bg-slate-100" />
                  <p className="micro-text text-muted-foreground font-medium italic opacity-40 tracking-tight leading-tight">{skill.description}</p>
                </div>
              ))}
            </div>
          </CardContent>
        ...
        <Card className="border border-border bg-white shadow-sm rounded-lg border-t-2 border-primary overflow-hidden">
          <div className="bg-slate-50/50 border-b border-border py-6 px-10">
            <div className="flex items-center gap-3">
              ...
              <h3 className="page-title text-[20px] text-foreground italic">Next Steps</h3>
            </div>
          </div>
            ...
            <div className="space-y-6">
              <div className="p-8 bg-slate-50/50 border border-border/60 rounded-md space-y-6 relative overflow-hidden group">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="micro-text text-primary font-semibold uppercase tracking-[0.2em]">Learning Advice</span>
                </div>
                <p className="text-[15px] leading-[1.7] text-foreground font-medium italic opacity-80">
                  Your performance in **Syntax** and **Usage** is strong, but you should practice more advanced sentence structures to reach the next level.
                </p>
              </div>

              <div className="space-y-5">
                <div className="micro-text font-semibold uppercase tracking-[0.25em] text-muted-foreground opacity-30 px-2">Areas for Improvement</div>
                  ...
                  {mockSkills.slice(2).map((skill) => (
                    <div key={skill.name} className="flex items-center justify-between p-4 px-6 border border-border/40 rounded-md bg-slate-50/20 hover:bg-slate-50 hover:border-border transition-all group">
                      <div className="flex items-center gap-4">
                         ...
                         <span className="text-[13px] font-semibold text-foreground opacity-70 uppercase tracking-tighter">{skill.name}</span>
                      </div>
