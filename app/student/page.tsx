...
export default function StudentPortal() {
  ...
  const handleSearchTest = () => {
    if (studentId && level && slot) {
      ...
      setTimeout(() => {
        setTestFound({
          title: `${level} - Trimester Assessment Protocol`,
          duration: 45,
          totalQuestions: 10,
      ...
      <div className="max-w-md w-full space-y-12 animate-in fade-in slide-in-from-bottom-2 duration-700">
        ...
        <div className="flex flex-col items-center text-center space-y-8">
          ...
          <div className="space-y-3">
            <h1 className="page-title text-[42px] font-medium text-[#0F172A] tracking-tighter leading-none italic underline decoration-primary/20 underline-offset-[14px]">
              Student <span className="opacity-40 not-italic">PORTAL</span>
            </h1>
            <p className="micro-text text-[#64748B] font-black uppercase tracking-[0.3em] opacity-40 mt-6 block">
              Institutional Evaluation Gateway
            </p>
          </div>
        </div>
        ...
        <Card className="border border-[#E2E8F0] bg-white shadow-lg rounded-lg overflow-hidden relative border-t-4 border-t-primary">
          <CardHeader className="text-center pb-8 border-b border-[#E2E8F0] bg-slate-50/50 pt-10">
            <CardTitle className="text-[20px] font-semibold text-[#0F172A] italic flex items-center justify-center gap-3 tracking-tight">
               <ShieldCheck className="h-4.5 w-4.5 text-primary opacity-60" />
              {testFound ? "Assessment Retrieval" : "Credential Verification"}
            </CardTitle>
            <CardDescription className="micro-text text-[#64748B] font-black uppercase tracking-widest opacity-40 mt-1.5">
              {testFound 
                ? "Academic Protocol Identified"
                : "Entry Protocol 2026.4"
              }
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-10 p-10 sm:p-12">
            {!testFound ? (
              <div className="space-y-8">
                ...
                <div className="space-y-4">
                  <Label htmlFor="id" className="micro-text text-[#64748B] font-black uppercase tracking-widest flex items-center gap-2 opacity-60">
                  ...
                  <Input
                    ...
                    className="h-12 rounded-md border-[#E2E8F0] bg-slate-50/30 focus-visible:ring-primary/10 text-[16px] font-black italic tracking-widest tabular-nums placeholder:italic placeholder:font-medium placeholder:opacity-30"
                  />
                </div>
                ...
                <div className="grid grid-cols-1 gap-8">
                  {/* Academic Level Node */}
                  <div className="space-y-4">
                    <Label htmlFor="level" className="micro-text text-[#64748B] font-black uppercase tracking-widest flex items-center gap-2 opacity-60">
                  ...
                  <div className="space-y-4">
                    <Label htmlFor="slot" className="micro-text text-[#64748B] font-black uppercase tracking-widest flex items-center gap-2 opacity-60">
                ...
                <Button 
                  className="btn-primary w-full h-14 text-[13px] font-black uppercase tracking-[0.25em] gap-4 shadow-md mt-4 btn-interactive"
              ...
              <div className="space-y-10 animate-in zoom-in-95 fade-in duration-500">
                <div className="p-10 rounded-md bg-slate-50 border border-[#E2E8F0] space-y-8 relative overflow-hidden group">
                  ...
                  <div className="flex justify-between items-start border-b border-[#E2E8F0] pb-8 relative z-10">
                    <div className="space-y-2">
                      <h4 className="text-[24px] font-semibold text-[#0F172A] italic decoration-primary/20 underline underline-offset-8 decoration-2">{testFound.title}</h4>
                      <p className="micro-text text-[#64748B] font-bold uppercase tracking-widest opacity-40 mt-3">{testFound.subject}</p>
                    </div>
                    <Badge variant="outline" className="border-primary/40 text-primary bg-primary/5 text-[11px] font-black px-4 h-8 tracking-widest uppercase italic tabular-nums">{testFound.duration} MIN</Badge>
                  </div>
                  <div className="grid grid-cols-2 gap-10 relative z-10">
                    <div className="text-left">
                      <p className="micro-text text-[#64748B] font-black uppercase tracking-widest opacity-40">Candidate Identity</p>
                      <p className="text-[17px] font-black tabular-nums mt-1.5 font-sans">{studentId}</p>
                    </div>
                    <div className="text-left">
                      <p className="micro-text text-[#64748B] font-black uppercase tracking-widest opacity-40">Spatial Allocation</p>
                      <p className="text-[17px] font-black tabular-nums mt-1.5 font-sans italic opacity-80">Slot {slot}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-5">
                  <Button 
                    className="btn-primary w-full h-16 text-[15px] font-black uppercase tracking-[0.3em] gap-5 shadow-lg active:scale-95 transition-all btn-interactive"
                  ...
                  <Button 
                    variant="ghost" 
                    className="w-full micro-text font-black text-[#64748B] uppercase h-12 hover:bg-slate-100/50 opacity-40 hover:opacity-100 transition-all tracking-[0.2em] italic"
            ...
            <p className="micro-text text-center text-[#64748B] pt-8 border-t border-[#E2E8F0] font-black opacity-20 uppercase tracking-[0.15em] italic">
               Institutional Gateway Protocol v2.4 
            </p>
          </CardContent>
        </Card>
      </div>
