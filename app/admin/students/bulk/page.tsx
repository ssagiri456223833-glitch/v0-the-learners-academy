...
export default function BulkUploadPage() {
  ...
  return (
    <DashboardLayout 
      ...
      subtitle="Registry Management • Direct Data Import Setup"
    >
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          {parsedStudents.length > 0 && (
             <Button 
               ...
               className="btn-primary h-12 px-8 gap-3 text-[11px] font-semibold uppercase tracking-widest shadow-md"
             >
              ...
              <Card className="border border-border bg-white shadow-sm rounded-lg overflow-hidden">
                 ...
                 <CardContent className="p-10 space-y-8">
                    <div className="space-y-4">
                       <Label className="micro-text font-semibold uppercase tracking-widest text-muted-foreground opacity-60">CSV / Raw List Input</Label>
                       <textarea 
                    ...
                    <div className="flex gap-4">
                       <Button 
                         variant="outline" 
                         className="flex-1 h-12 border-border text-[11px] font-semibold uppercase tracking-widest opacity-60 hover:opacity-100"
                         onClick={() => { setRawData(""); setParsedStudents([]); }}
                       >
                          Clear Stream
                       </Button>
                       <Button 
                         className="btn-primary flex-1 h-12 text-[11px] font-semibold uppercase tracking-widest gap-3"
              ...
              <Card className="border border-border bg-slate-50/50 shadow-inner rounded-lg overflow-hidden">
                 <CardContent className="p-8 flex items-start gap-6">
                    ...
                    <div className="space-y-2">
                       <h4 className="text-[14px] font-semibold text-foreground">Validation Checks</h4>
                       <p className="text-[13px] text-muted-foreground leading-relaxed">
                          Ensure Student IDs match the institutional pattern (e.g., L-XXXX). Duplicate records will be flagged during the final commitment phase.
                       </p>
                    </div>
                 </CardContent>
              </Card>
           ...
           <div className="space-y-8">
              <Card className="border border-border bg-white shadow-sm rounded-lg overflow-hidden h-full flex flex-col">
                 <CardHeader className="bg-slate-50 border-b border-border p-10 flex flex-row items-center justify-between shrink-0">
                    ...
                    <Badge variant="outline" className="border-primary/20 bg-primary/5 text-primary text-[10px] font-semibold px-3 h-6 uppercase">{parsedStudents.length} Students</Badge>
                 </CardHeader>
                 <CardContent className="p-0 flex-1 overflow-hidden relative">
                    {!parsedStudents.length ? (
                       <div className="absolute inset-0 flex flex-col items-center justify-center p-12 text-center">
                          ...
                          <p className="text-[14px] font-medium text-muted-foreground opacity-40 italic">Waiting for validated list data...</p>
                       </div>
                    ) : (
                       <div className="overflow-auto h-[600px] custom-scrollbar">
                          <table className="w-full text-left border-collapse">
                             <thead className="sticky top-0 bg-white border-b border-border z-10 shadow-sm">
                                <tr>
                                   <th className="p-6 micro-text text-muted-foreground font-semibold uppercase opacity-60">Student Name</th>
                                   <th className="p-6 micro-text text-muted-foreground font-semibold uppercase opacity-60">Student ID</th>
                                   <th className="p-6 micro-text text-muted-foreground font-semibold uppercase opacity-60">Level</th>
                                   <th className="p-6 micro-text text-muted-foreground font-semibold uppercase opacity-60">Timing</th>
                                </tr>
                             </thead>
                             <tbody className="divide-y divide-slate-50">
                                {parsedStudents.map((s, i) => (
                                  <tr key={i} className="hover:bg-slate-50 transition-colors">
                                     ...
                                     <td className="p-6">
                                        <Badge variant="secondary" className="text-[9px] font-semibold px-2 h-5 uppercase tracking-widest">{s.level}</Badge>
                                     </td>
        ...
        {status === "success" && (
           <div className="fixed bottom-12 right-12 animate-in slide-in-from-bottom-4 duration-500 z-50">
              <div className="bg-success text-white px-8 py-4 rounded-md shadow-2xl flex items-center gap-4 border border-success/30">
                 ...
                 <span className="text-[14px] font-semibold uppercase tracking-widest">Registry Sync Successful</span>
              </div>
           </div>
        )}
      </div>
